...
/**
 * Stand-alone DKIM class to sign email with a 2048 bit private key hashed with SHA-256 algorithm. This stand-alone DKIM class is based on the work made by Louis Ameline and PHP-MAILER (see license below).
 *
 * **The differences are :**
...
class DKIMsign
{
	...
	/**  @var string $private_key Provide your 2048 bit private key generated in Terminal with command line: ```openssl genrsa -des3 -out private.pem 2048``` */
	private $private_key;
	...
	private function _dkim_canonicalize_headers_relaxed($sHeaders)
	{
		...
		foreach ($lines as $key => $line) {
			...
			if (!empty($line)) {
				...
				if (
					in_array($header_type, $this->options['signed_headers'])
